...
//        NSString *d2 = [[NSString alloc] initWithData:[dTextIn gzipInflate] encoding:NSUTF8StringEncoding];
//        dTextIn = [[JoDes decodeBase64WithString:d2] mutableCopy];
        dTextIn = [[dTextIn gzipInflate] mutableCopy];
        ...
        iv = [[NSData dataWithBytes:((char *)dTextIn.bytes) + 0
                                             length:kCCBlockSizeDES] mutableCopy];
        [dTextIn replaceBytesInRange:NSMakeRange(0, kCCBlockSizeDES) withBytes:NULL length:0]; // 移除iv位
        ...
        iv = [[JoDes randomCreateIv] mutableCopy];
//        iv = [[@"a2345678" dataUsingEncoding:NSUTF8StringEncoding] mutableCopy];
        [iv setLength:kCCBlockSizeDES];
    }
    ...
    if (encryptOrDecrypt == kCCDecrypt){
        ...
        free(bufferPtr1);
        // 将向量补在数据之前
        NSMutableData *dResult2 = [[NSMutableData alloc] initWithData:iv];
        [dResult2 appendData:dResult];
        // 压缩
        dResult = [dResult2 gzipDeflate];
        // base64
...
+ (NSData *)randomCreateIv{
    ...
    for (int i = 0; i < numberOfCharacters; i++) {
        NSUInteger j = (arc4random_uniform((unsigned int)numberOfCharacters - i) + i);
    ...
    if (data.length < 8) {
        data = [JoDes randomCreateIv];
    }
    return [data subdataWithRange:NSMakeRange(0, kCCBlockSizeDES)];
}
